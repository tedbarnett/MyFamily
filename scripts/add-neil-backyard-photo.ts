...
async function main() {
  ...
  const imagePath = 'attached_assets/generated_images/neil_relaxing_in_backyard.png';
  ...
  // Include primary photo if not already in array
  if (currentPerson.photoData && !currentPhotos.includes(currentPerson.photoData)) {
    currentPhotos.unshift(currentPerson.photoData);
  }
  // Add new photo
  ...
  console.log(`Added backyard photo to Neil Hampton. Total photos: ${updatedPhotos.length}`);
  process.exit(0);
}
